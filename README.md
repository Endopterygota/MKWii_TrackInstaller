...
- **Track project** — project folder, content-detected track-files folder, SZS dropdown, folder-structure generator, WiiScrubber, and ISO target configuration
- **SZS tools** — background `wszst create` and `wszst check` surfaces
- **Settings** — language, Dolphin, run controls, and desktop mode
...
Die EXE enthält die benötigte Electron-Laufzeit und das native Automatisierungs-Backend. Eine Installation ist nicht erforderlich. WiiScrubber, Dolphin und Wiimms SZS Tools werden weiterhin über die in der App gewählten lokalen Pfade beziehungsweise den Windows-PATH verwendet.

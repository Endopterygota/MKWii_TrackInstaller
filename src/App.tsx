...
export default function App() {
  ...
  const [szsFiles, setSzsFiles] = useState<string[]>([]);
  ...
  const [structureBasePath, setStructureBasePath] = useState("");
  ...
  const [szsComparison, setSzsComparison] = useState<SzsComparison | null>(null);
  ...
  const desktopConfig = useMemo<DesktopConfig>(() => ({ ...config, dolphin, wit, iso, testIso, language }), [config, dolphin, wit, iso, testIso, language]);

  useEffect(() => {
    setSzsComparison(null);
  }, [config.trackFolder, config.trackFilesFolder]);
  ...
  const dialogCopy = useMemo(() => {
    ...
    if (dialog === "wit-install") return { title: en ? "Install track?" : "Strecke einsetzen?", description: en ? "WIT creates the selected test ISO on the first run and updates the same file on later runs." : "WIT legt beim ersten Durchlauf die ausgewählte Test-ISO an und aktualisiert bei späteren Durchläufen dieselbe Datei.", icon: <DatabaseZap size={18} />, action: en ? "Install now" : "Jetzt einsetzen" };
    if (dialog === "wit-play") return { title: en ? "Install & launch?" : "Einsetzen & starten?", description: en ? "WIT updates the selected test ISO and Dolphin launches that exact file." : "WIT aktualisiert die ausgewählte Test-ISO und Dolphin startet genau diese Datei.", icon: <Gamepad2 size={18} />, action: en ? "Launch with Dolphin" : "Mit Dolphin starten" };
    if (dialog === "full-run") return { title: en ? "Build, install and launch?" : "Erstellen, einsetzen und starten?", description: en ? `wszst first creates ${trackTarget}.szs, WIT then installs it into the selected test ISO, and Dolphin launches that ISO.` : `Zuerst erstellt wszst die ${trackTarget}.szs, danach setzt WIT sie in die ausgewählte Test-ISO ein und Dolphin startet genau diese ISO.`, icon: <Rocket size={18} />, action: en ? "Run all three steps" : "Alle drei Schritte starten" };
    return null;
  ...
  async function runCommand(command: AutomationCommand) {
    ...
    try {
      ...
      if (result.ok && (command === "wszst-create" || command === "build-wit-install-play")) {
        setSzsComparison(result.szsComparison);
  ...
  async function browse(kind: "folder" | "structure-base" | "scrubber" | "dolphin" | "wit" | "iso" | "test-iso") {
    const api = window.mkwii;
    const currentPaths = { folder: config.trackFolder, "structure-base": structureBasePath, scrubber: config.scrubber, dolphin, wit, iso, "test-iso": testIso };
    const selected = await api?.choosePath(kind, currentPaths[kind]);
    if (!selected) return;
    if (kind === "dolphin") {
      setDolphin(selected);
      ...
      setIso(selected);
      await api?.saveConfig({ ...desktopConfig, iso: selected });
      addLog("OK", language === "en" ? "Original ISO path saved." : "Pfad der Original-ISO gespeichert.");
    }
    else if (kind === "test-iso") {
      setTestIso(selected);
      await api?.saveConfig({ ...desktopConfig, testIso: selected });
      addLog("OK", language === "en" ? "Test ISO path saved." : "Pfad der Test-ISO gespeichert.");
    }
  ...
  return (
    <AppShell page={page} onPageChange={setPage} language={language} onLanguageToggle={toggleLanguage} consolePanel={<AutomationConsole logs={logs} running={busy} paused={paused} language={language} checkWarnings={showCheckWarnings ? checkWarnings : []} additionalFiles={additionalFiles} onDeleteAdditionalFiles={() => setDeleteDialogOpen(true)} onDismissWarnings={() => { warningsDismissedRef.current = true; setShowCheckWarnings(false); }} />}>
      ...
      <Dialog open={dialog !== null} title={dialogCopy?.title ?? ""} description={dialogCopy?.description} closeLabel={language === "en" ? "Close dialog" : "Dialog schließen"} onClose={() => setDialog(null)}>
        <div className="dialog-summary"><span>{dialogCopy?.icon}</span><div><b>{trackTarget}.szs</b><small>{config.targetFile} · {dialog?.startsWith("wit") || dialog === "full-run" ? testIso : iso}</small></div></div>
      ...
      <Dialog open={deleteDialogOpen} title={language === "en" ? "Delete additional files?" : "Zusätzliche Dateien löschen?"} description={language === "en" ? `This permanently deletes every file reported by wszst as 'HINT: Additional file'. This cannot be undone.` : `Dadurch werden alle von wszst als \u201EHINT: Additional file\u201C gemeldeten Dateien endg\u00FCltig gel\u00F6scht. Dies kann nicht r\xFCckg\u00E4ngig gemacht werden.`} closeLabel={language === "en" ? "Close dialog" : "Dialog schließen"} onClose={() => !deleteBusy && setDeleteDialogOpen(false)}>

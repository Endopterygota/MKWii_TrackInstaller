import { CircleCheck, Disc3, FileArchive, Folder, FolderPlus, Info, MapPin, Save, ShieldAlert, TriangleAlert } from "lucide-react";
...
type ProjectPageProps = {
  ...
  onSave: () => void;
  onBrowse: (kind: "folder" | "scrubber") => void;
    ...
    <div className="page">
      ...
      <Card className="form-card">
        ...
        <div className="form-grid">
          ...
          <Field label="WiiScrubber" value={config.scrubber} icon={<Disc3 size={16} />} onChange={(e) => set("scrubber", e.target.value)} action={<button className="field-action" disabled={busy} onClick={() => onBrowse("scrubber")}>{en ? "Choose" : "Auswählen"}</button>} />
      ...
      <Card className="structure-card">
        ...
        <div className="form-grid structure-form">
          <Field label={en ? "Project / track name" : "Projektname / Streckenname"} value={structureName} icon={<FolderPlus size={16} />} onChange={(event) => onStructureNameChange(event.target.value)} hint={en ? "Example: GoombaPark" : "Beispiel: GoombaPark"} />
      ...
      <div className="info-grid">
        ...
        <Card className="notice-card notice-card--warning"><span><ShieldAlert /></span><div><h3>{en ? "ISO notice" : "ISO-Hinweis"}</h3><p><code>MarioKartWii [RMCP01].iso</code> {en ? "must be located in the Windows Documents folder. The ISO is modified directly." : "muss im Windows-Ordner „Dokumente“ liegen. Die ISO wird direkt verändert."}</p></div></Card>
      </div>
    </div>
  );
}

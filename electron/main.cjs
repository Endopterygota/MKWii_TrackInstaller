...
const commonWitPath = "C:\\Program Files\\Wiimm\\WIT\\wit.exe";
...
const defaultConfig = {
  ...
  dolphin: "C:\\Users\\Linus\\Documents\\Dolphin-x64\\Dolphin.exe",
  wit: fs.existsSync(commonWitPath) ? commonWitPath : "wit.exe",
  language: "de",
};
...
function sanitizeConfig(value) {
  ...
  if (clean.wit.toLowerCase() === "wit.exe" && fs.existsSync(commonWitPath)) clean.wit = commonWitPath;
  return clean;
}
...
async function startAutomation(command, rawConfig) {
  ...
  return await new Promise((resolve) => {
    ...
    child.once("close", async (code) => {
      ...
      await fsp.rm(controlDir, { recursive: true, force: true }).catch(() => {});
      emit("automation:state", { running: false, paused: false, command });
      resolve({ ok: code === 0, stopped: code === 2, exitCode: code ?? -1 });
    });
  });
}
...
async function choosePath(kind) {
  ...
  const filters = [{ name: "Programme", extensions: ["exe"] }];
...
function registerIpc() {
  ...
  ipcMain.handle("dialog:choose-path", (_event, kind) => choosePath(kind));
